...
const CategoriesApp = (props) => {
  ...
  return (
    <Fragment>
      <Breadcrumb parent="Tablero" title="CATEGORIAS" />
                  ...
                  <CardBody>
                    <div className="email-app-sidebar left-bookmark">
                      <Nav className="main-menu" role="tablist">
                        <NavItem>
                          <Button color="primary" className="btn-block btn-mail badge-light-primary" onClick={addToggle}>{`Nuevo Categoria`}</Button>
                          <Modal isOpen={addModal} toggle={addToggle} size="lg">
                            <ModalHeader toggle={addToggle}>Nueva categoria</ModalHeader>
                            <ModalBody>
                              <Form className="form-bookmark needs-validation" onSubmit={handleSubmit(Addtask)}>
                                <div className="form-row">
                                  ...
                                  <FormGroup className="col-md-6">
                                    <Label>{"Color"}</Label>
                                    <input className="form-control" name="desc" type="color" {...register('desc', { required: true })}></input>
                        ...
                        <NavItem>
                          <a href="#javaScript" className={activeTab === -1 ? 'active' : ''} onClick={() => setActiveTab(-1)}>
                            <span className="title">Todas las Categorias ({newtaskdata.length})</span>
                          </a>
                        </NavItem>  
                      </Nav>
                    </div>
